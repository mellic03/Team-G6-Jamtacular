class Player {
    ...
    control() {
        ...
        if (this.ALIVE == true) {
            ...
            this.rayCastGameplayMechanic(mainMap);
        }
    ...
    rayCastGameplayMechanic(main_map) {

        if (main_map.type == "raycasted") {
            if (keyIsDown(16)) {
        
                main_map.rayCast.draw(this.sprite.position.x, this.sprite.position.y);
                main_map.rayCast.cast(boundaries);
    
                for (let block of main_map.blocks) {
                    block.visible = false;
                }
                for (let door of main_map.doors) {
                    door.visible = false;
                }
                this.health -= 0.001;
